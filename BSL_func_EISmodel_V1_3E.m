function [output,paras] = BSL_func_EISmodel_V1_3E(f_vector,factors,soc,T,type_acf)
...
if type_acf ~=3
    error('change function to V1_half model')
end
...
%% Kinetics Parameters
    ...
    Rpa =  (17.8)*1e-6;              Rpc = (10)*1e-6;     % [um] Radius of particles  % LGES 2024 02, 05 3E
    Dsa = factors(4)*Dsa_function(x,T);      Dsc = factors(9)*Dsc_function(y,T); % {modified} [m^2/sec], 05 3E      
    Cdla =  factors(3)*0.2;             Cdlc = factors(8)*0.2;             % [F/m2]     , 05 3E
    cta = 29626;                        ctc = 48786;            % [mol/m3]

    % Exchange current density
    ka = factors(2)*ka_function(x,T,cta); % {modified}
    kc = factors(7)*kc_function(y,T,ctc); % {modified}, 05 3E
...
elseif type_acf ==3 % full cell
    ...
    error('select anode (1), cathode (2), full cell_3E (3)')
end
