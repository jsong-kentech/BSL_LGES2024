function [z_fc,used_para,z_local] = JS_EIS_model_V6b(w_vector,factors,soc_ref,T_ref)
...
% [v6] adjusted for fitting - taking fit parameters as input, and output is the full cell impedance in a form of real matrix
    % [v6] Factors
        % factors = [R_itsc,brug,p_Ds,n_Ds,p_i0,p_Cdl,p_dUdc,n_dUdc,p_av,n_av,n_Cdl,n_i0] [1x9]
    % [v6] Additional parameters
        A_coat = 0.070452; % [m2]
        R_itsc = factors(1)*0.0264; % [Ohm] 
% [v6b] (1) incorporate the particle size distribution
%       (2) replicate the comsol parameters (anode films) (indicated by *+*)
%       (3) added one more fitting parameter
...
% [v6b] particle size distribution
mean_ra = 1;                        mean_rc = 1;
std_ra = 0.3;                       std_rc = 0.3;      % [*+*] thse are parameters for observed lognormal distribution, dimensionless
...
Cdla =  factors(9)*0.2;                        Cdlc = factors(6)*0.2;             % [F/m2]     
...
ka = factors(10)*ka_function(theta0a,T,cta); % {modified}
...
z_fc = [R_itsc+(1/A_coat)*real(fc_imp.'),(1/A_coat)*imag(fc_imp.')]; % unit is [Ohm] format of real matrix
used_para = [R_itsc, bruga, Dsc, Dsa, i0c, Cdlc, dUdcc, dUdca] ;
z_local = [w_vector, fc_imp.', c_imp.', a_imp.', s_imp.'];
end
